...
const ProjectModal = ({ project, onClose }: { project: ProjectCardItem; onClose: () => void }) => {
  ...
  return (
    ...
    >
      <div
        className={`relative flex flex-col overflow-hidden rounded-[1.2rem] bg-card shadow-modal ${
          hasHtmlDetail
            ? 'h-[96vh] w-[98vw] max-w-[118rem]'
            : 'h-[72vh] w-full max-w-6xl'
        }`}
      ...
      >
        ...
        {hasHtmlDetail ? (
          ...
          <div className="h-full overflow-hidden p-4 md:p-5">
            <div className="grid h-full min-h-0 items-start gap-4 xl:grid-cols-[0.95fr_1.05fr]">
        ...
        <section className="pb-16 pt-8">
          <div className="sticky top-16 z-30 border-b border-border/50 bg-background">
            <AnimatedSection animation="fade-up" className="container mx-auto px-4 py-5">
              <div
                className={`mx-auto max-w-7xl overflow-hidden rounded-[2rem] border border-border bg-card shadow-sm transition-all duration-300 ${
                  isFilterOpen ? 'pb-4' : ''
                }`}
              >
                <div className="flex min-h-14 items-center justify-between px-5 py-3">
                  <button
                    type="button"
