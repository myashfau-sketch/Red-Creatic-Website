...
import ProjectsPageClient, { type ProjectCardItem } from './ProjectsPageClient';
import { notFound } from 'next/navigation';

const fallbackProjects: ProjectCardItem[] = [
  {
    id: 'fallback-1',
    title: 'Hulhumale Retail Branding',
    description:
      'Complete branding overhaul for a retail clothing store in Hulhumale. The project included full storefront signage, interior wall graphics, window decals, and branded shopping bags.',
    detailHtmlUrl: null,
    customer: {
      name: 'Ahmed Rasheed',
      business: '2024',
  ...
  },
  {
    ...
    description:
      'Full vehicle wrap project for a luxury resort transportation fleet using premium vinyl suitable for tropical and saltwater exposure.',
    detailHtmlUrl: null,
    customer: {
  ...
  },
  {
    ...
    description:
      'Comprehensive office signage and branding for a financial services company relocating to a new headquarters.',
    detailHtmlUrl: null,
    customer: {
      name: 'Mariyam Shifa',
      business: '2025',
...
function mapProjectRecord(record: ProjectRecord): ProjectCardItem | null {
  ...
  return {
    id: record.id,
    title: record.title,
    description: record.description || 'Open the project to view the full detail.',
    detailHtmlUrl: record.detail_html_url || null,
    customer: {
      name: record.client_name || 'Client',
      business: record.completion_year || 'Project',
...
export default async function ProjectsPage({
  ...
  if (!(await isSitePageEnabled('projects', searchParams))) {
    notFound();
  }
  ...
  try {
    ...
    if (!error && data && data.length > 0) {
      const mappedProjects = (data as ProjectRecord[])
      ...
      if (mappedProjects.length > 0) {
        ...
        const derivedServices = Array.from(
          new Set(mappedProjects.flatMap((project) => project.works).filter(Boolean))
        ).sort();
        const derivedProducts = Array.from(
          new Set(mappedProjects.flatMap((project) => project.products).filter(Boolean))
        ).sort();
    ...
    // Fall back to existing project data if Supabase is unavailable.
  }
