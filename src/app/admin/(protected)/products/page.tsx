...
export default async function AdminProductsPage() {
  ...
  const items = (data ?? []) as ProductRecord[];
        ...
        <div className="rounded-[1.5rem] border border-border bg-card px-5 py-4 shadow-sm">
          <div>
            ...
            <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <h1 className="text-[1.75rem] font-semibold text-foreground">Products</h1>
                <p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">Manage product content from a wider backend workspace with navigation on the left and details on the right.</p>
              </div>
              <div className="flex items-center gap-3">
                ...
                <div className="rounded-xl bg-primary/10 px-4 py-2 text-sm font-medium text-primary">{items.length} products</div>
              </div>
            </div>
          </div>
        </div>

        <AdminProductsManager items={items} upsertAction={upsertProduct} deleteAction={deleteProduct} />
      </div>
    </main>
  );
}
